...
function resizeImageBlob(file, maxWidth = 1200, quality = 0.85) {
...
export default function ImageToPdf() {
  ...
  const handleFiles = (e) => {
    ...
    if (chosen.length > 60) chosen.length = 60;
    setFiles(chosen);
  };
  ...
  const makePdf = async () => {
    if (!files.length) { setError("Choose images first."); return; }
  ...
  return (
    <div>
      <div className="controls" aria-live="polite">
        <div className="row">
          <label className="small">Max width (px)
            <input type="number" value={maxWidth} onChange={(e)=>setMaxWidth(Number(e.target.value || 1200))} min="200" />
          </label>
          <label className="small">Quality (JPEG 0.1 - 1)
            <input type="number" step="0.05" min="0.1" max="1" value={quality} onChange={(e)=>setQuality(Number(e.target.value || 0.85))} />
          </label>
        </div>

        <div className="row">
          <input id="fileInputPdf" type="file" accept="image/*" multiple onChange={handleFiles} />
        </div>

        <div className="row" style={{ marginTop: 6 }}>
          <button className="primary" onClick={makePdf} disabled={processing || !files.length}>{processing ? "Processing..." : "Download PDF"}</button>
      ...
      <div style={{ marginTop: 14 }}>
        ...
        {files.length===0 ? <div className="notice">No images selected yet.</div> :
